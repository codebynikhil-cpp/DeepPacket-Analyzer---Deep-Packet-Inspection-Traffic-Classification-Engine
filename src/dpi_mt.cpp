...
private:
    ...
    void run() {
        while (running_) {
            ...
            // Hash to select FP - shift right to avoid colliding with the LB selection hash (which uses lower bits)
            FiveTupleHash hasher;
            size_t hash_val = hasher(pkt_opt->tuple);
            size_t fp_idx = (hash_val >> 16) % num_fps_;
